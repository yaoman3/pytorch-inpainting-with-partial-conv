...
class DIV2K(torch.utils.data.Dataset):
    def __init__(self, img_root, mask_root, img_transform, mask_transform,
        ...
        # use about 8M images in the challenge dataset
        if split == 'train':
            self.paths = glob('{:s}/DIV2K_train_HR/*.png'.format(img_root),
                              recursive=True)
            print('Dataset Path: ' + '{:s}/DIV2K_train_HR/*.png'.format(img_root))
        else:
            self.paths = glob('{:s}/DIV2K_valid_HR/*.png'.format(img_root))
            print('Dataset Path: ' + '{:s}/DIV2K_valid_HR/*.png'.format(img_root))
